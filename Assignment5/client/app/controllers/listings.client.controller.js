angular.module('listings').controller('ListingsController', ['$scope', '$location', '$stateParams', '$state', 'Listings', 
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.initEdit = function() {
      $scope.findOne()
        .then(function() {
          $scope.listingEdit = angular.copy($scope.listing);
        });
    };
    ...
    $scope.onMarkerClickedListener = function(marker, eventName, model) {
      console.log(model)
    ...
    $scope.update = function(isValid) {
      ...
      Listings.update($scope.listingEdit._id, $scope.listingEdit)
              .then(function(response) {
                //if the object is successfully saved redirect back to the list page
                $state.go('listings.list', { successMessage: 'Listing succesfully created!' });
              }, function(error) {
